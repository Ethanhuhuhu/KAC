    ...
    def load(self):
        ...
        if self.train or self.validation:
            for domain in domains:
                data = []
                target = []
                image_list = open(os.path.join('data/DomainNet',domain,domain+'_train.txt')).readlines()
                for image in image_list:
                    image_path = os.path.join('data/DomainNet', image.split()[0])
            ...
            for domain in domains:
                data = []
                target = []
                image_list = open(os.path.join('data/DomainNet',domain,domain+'_test.txt')).readlines()
                for image in image_list:
                    image_path = os.path.join('data/DomainNet', image.split()[0])
                    data.append(image_path)
                    self.data.append(image_path)
